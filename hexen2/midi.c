...
/* open a MIDI file. Then it goes tabout converting at least the first part  */
/* of that file into a midiStream buffer for playback.			     */
/*****************************************************************************/
...
void CALLBACK MidiProc(HMIDIIN hMidi,UINT uMsg,DWORD dwInstance,DWORD dwParam1,DWORD dwParam2)
{
	...
	switch(uMsg)
	{
		case MOM_DONE:
			...
			// This flag is set whenever the callback is
			// waiting for all buffers to come back.
			if(uCallbackStatus == STATUS_KILLCALLBACK) {
				...
				// Then send a stop message when we get
