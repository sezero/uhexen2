/*
	...
	$Header: /home/ozzie/Download/0000/uhexen2/hexen2/common.c,v 1.11 2005-05-17 22:56:19 sezero Exp $
*/
...
void FindEncryption(void)
{
	...
	while(strlen(Position) != 0)
	{
		...
		if (DriveType == DRIVE_CDROM)
		{
			...
			if (Result && Q_strcasecmp(VolumeName,"Hexen II") == 0)
...
//			if (Result && Q_strcasecmp(VolumeName,"Hexen II") == 0)
//			if (Result && Q_strcasecmp(VolumeName,"m3D_2") == 0)
//			if (Result && Q_strcasecmp(VolumeName,"dkreign") == 0)
			if (Result && Q_strcasecmp(VolumeName,"H2MP") == 0)
			    //&& VolumeSerialNumber == 0x24214a8b)
			{
				FILE *FH;
				char temp[MAX_PATH];
...
void DecryptFile(char *FileName, byte *Buffer, int BufSize)
{
	...
	for(i=0;i<MAX_LIST;i++)
		if (Q_strcasecmp(&FileName[Length-4],FileList[i]) == 0)
			break;
