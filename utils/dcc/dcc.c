...
char* type_names[8] = {"void", "string", "float", "vector", "entity", "ev_field", "void()", "ev_pointer"};
  ...
  s->op %= 100;
	...
	if(pr_dumpasm) {
		 ...
		 PR_Print("\n%s(%d): %s(%d) %s(%d) %s(%d):\n",pr_opcodes[s->op].opname,s->op,arg1,
			  (signed short)s->a,arg2,(signed short)s->b,arg3,(signed short)s->c);
	}
  arg1 = arg2 = arg3 = NULL;
...
*/
	 ...
	 if(arg3 && nargs>1){
		 ...
		 if (!strcmp(fnam,"WriteCoord") || !strcmp(fnam,"WriteAngle"))
			  if(strcmp(&arg3[strlen(arg3)-2],"_x") && !(!strcmp(&arg3[strlen(arg3)-2],"_y") || !strcmp(&arg3[strlen(arg3)-2],"_z") || !strcmp(&arg3[strlen(arg3)-1],")")))
				  strcat(line,"_x");
	 }
	 for(i = 2;i<nargs;i++){
		 strcat(line,", ");
...
//			  if(strcmp(&arg1[strlen(a1)-2],"_x") && !(!strcmp(&a1[strlen(arg1)-2],"_y") || !strcmp(&a1[strlen(arg1)-2],"_z")))
//				  strcat(line,"_x");
//#endif
		 //if(arg1)
			 //free(arg1);
	 ...
	 for(i = 1;(s+i)->op;i++){
		...
		if((s+i)->a == OFS_RETURN || (s+i)->b == OFS_RETURN || (s+i)->c == OFS_RETURN){
		  j = 0;
		  //printf("ofs_return is used before another call %d %d %d\n",j,i,(s+i)->op);
		  break;
		}
		if(OP_CALL0  <= ((s+i)->op%100) && ((s+i)->op%100) <= OP_CALL8 ){
		  //if(i == 1){
		  //	  j = 0;
		  //}
...
void PR_Locals(dfunction_t *df){
	...
	for(j=start;j<df->parm_start+df->locals;j++){
		...
		if(par){
		  if(par->type==ev_void || par->type==ev_void || par->type==ev_field ||
			  par->type==ev_function || par->type==ev_pointer){
			  continue;
		  }
...
///typedef enum {ev_void, ev_string, ev_float, ev_vector, ev_entity, ev_field, ev_function, ev_pointer} etype_t;
		  switch (par->type){
			case  ev_string:   PR_Print("string");break;
			case  ev_void:   PR_Print("void");break;
			case  ev_float:   PR_Print("float");break;
			case  ev_vector:   PR_Print("vector");j+=2;break;
			case  ev_entity:   PR_Print("entity");break;
			case  ev_field:   PR_Print("ev_field");break;
			case  ev_function:   PR_Print("void()");break;
			case  ev_pointer:   PR_Print("ev_pointer");break;
		  }
...
void PR_FunctionHeader(dfunction_t *df){
	...
	if(df->first_statement < 0){       //builtin
...
//get return type
	...
	switch (t1){
		case  ev_string:   sprintf(line,"string ");break;
		case  ev_void:   sprintf(line,"void ");break;
		case  ev_float:   sprintf(line,"float ");break;
		case  ev_vector:   sprintf(line,"vector ");break;
		case  ev_entity:   sprintf(line,"entity ");break;
		case  ev_field:   sprintf(line,"ev_field ");break;
		case  ev_function:   sprintf(line,"void() ");break;
		case  ev_pointer:   sprintf(line,"ev_pointer ");break;
		default:   sprintf(line,"void ");break;
	}
...
//print parameters
	...
	for(j=0;j<df->numparms;j++){
	  ...
	  if(par){
		switch (par->type){
			case  ev_string:   strcat(line,"string ");break;
			case  ev_void:   strcat(line,"void ");break;
			case  ev_float:   strcat(line,"float ");break;
			case  ev_vector:   strcat(line,"vector ");break;
			case  ev_entity:   strcat(line,"entity ");break;
			case  ev_field:   strcat(line,"ev_field ");break;
			case  ev_function:   strcat(line,"void() ");break;
			case  ev_pointer:   strcat(line,"ev_pointer ");break;
		}
...
unsigned short GetReturnType(int func){
	...
	while (1)
	{
		...
		if ( ds->op == OP_RETURN ) {   //find 2 differnent returns if possible just to be certain(ie: findtarget)
		  k++;
		  if(ds->a != 0){
			  if (ds->a == OFS_RETURN) {
				  di = ds-1;
				  while((di-statements) >= df->first_statement){  //that stupid equal, what a bitch
				  ...
				  if(par)
					  rtype[k] = par->type;
				  else{ //find the op where the reg was created, it can tell what the val is
					  rtype[k] = ev_void;
...
unsigned short OP_StoreValue(dstatement_t *ds){
	 switch (ds->op){
		...
		case OP_CALL6:
		case OP_CALL7:
		case OP_CALL8:return ev_function;
		...
		case OP_MUL_FV:
		case OP_MUL_VF:
		case OP_STOREP_V:return ev_vector;
		...
		case OP_STORE_S:
		case OP_STOREP_S:
		case OP_NOT_S:return ev_string;
		...
		case OP_STORE_ENT:
		case OP_NOT_ENT:
		case OP_STOREP_ENT:return ev_entity;
		...
		case OP_LOAD_FNC:
		case OP_STORE_FNC:
		case OP_STOREP_FNC:return ev_function;

		case OP_LOAD_FLD:
		case OP_ADDRESS:
		case OP_STORE_FLD:
		case OP_STOREP_FLD:return ev_field;

		case OP_RETURN:
		case OP_NOT_FNC:
		case OP_DONE:
		case OP_GOTO:return ev_void;
	}
   return ev_void;
}
...
void PR_LocalGlobals(){
  ...
  for (j=start; j<end; j++) {
	 ...
	 bsize = CalcArraySize(j,end);
			...
			//					  par->ofs,par2?(par2->ofs-par->ofs):-1,bsize-par->ofs);
					...
					if ( strcmp(strings + par->s_name, IMMEDIATE_VALUE) ) {
                      ...
                      bsize = (*(int *)&pr_globals[par->ofs-1]) + 1;
						 ...
						 if (par->type == ev_entity || par->type == ev_void) {
							 ...
							 if(pr_dumpasm || printassign || !strcmp(strings + par->s_name,"string_null")){
							  if( bsize < 2)
		  ...
		  if(arg2){
				if(!strcmp(s_name,arg2)){
					if((d+1)->op == OP_STOREP_FNC){
						...
						if(strcmp("void  ()",func_headers[j])){ //if void () continue checking just to be certain (ie: th_pain)
...
//print results to string
		if(type[8] & (1<<15)){
		  ...
		  sprintf(plist,"");
		switch (type[8]){
			case  ev_string:   strcat(plist,"string ");break;
			case  ev_void:   strcat(plist,"void ");break;
			case  ev_float:   strcat(plist,"float ");break;
			case  ev_vector:   strcat(plist,"vector ");break;
			case  ev_entity:   strcat(plist,"entity ");break;
			case  ev_field:   strcat(plist,"ev_field ");break;
			case  ev_function:   strcat(plist,"void() ");break;
			case  ev_pointer:   strcat(plist,"ev_pointer ");break;
			default:   strcat(plist,"unknown ");break;
		}
	strcat(plist,"(");
	if(df->numparms < 0){
	  strcat(plist,"...");
	  ...
	  for(i=0;i<j;i++){
		...
		switch (type[i]){
			...
			case  ev_function:   strcat(plist,"void() ");break;
			case  ev_pointer:   strcat(plist,"ev_pointer ");break;
			default:   sprintf(parm,"unknown%d",type[i]);strcat(plist,parm);break;
		}
		sprintf(parm,"%c",'a'+i);
		strcat(plist,parm);
		if(i<j-1)
