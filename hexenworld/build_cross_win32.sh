...
make -C Server -f $MAKEFILE WIN32CC=$TARGET-gcc MINGWDIR=$TARGET $*

echo "" && echo "Building HexenWorld Master Server"
make -C Master -f $MAKEFILE WIN32CC=$TARGET-gcc MINGWDIR=$TARGET $*

echo "" && echo "Building HexenWorld Client (Software renderer)"
make -C Client -f $MAKEFILE WIN32CC=$TARGET-gcc WIN32AS=nasm WIN32RES=$TARGET-windres MINGWDIR=$TARGET $* hw_exe

echo "" && echo "Building HexenWorld Client (OpenGL renderer)"
make -C Client -f $MAKEFILE clean
make -C Client -f $MAKEFILE WIN32CC=$TARGET-gcc WIN32AS=nasm WIN32RES=$TARGET-windres MINGWDIR=$TARGET $* glhw_exe
