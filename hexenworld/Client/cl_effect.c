...
{	
	...
	switch(cl.Effects[index].type)
	{
		...
		case CE_TELESMK1:
			FreeEffectEntity(cl.Effects[index].u.Smoke.entity_index2);
		...
		case CE_ONFIRE:
		case CE_RIPPLE:
			FreeEffectEntity(cl.Effects[index].u.Smoke.entity_index);
			break;
		...
		case CE_RED_FLASH:
			FreeEffectEntity(cl.Effects[index].u.Flash.entity_index);
			break;
		...
		case CE_TELEPORTERPUFFS:
			for (i=0;i<8;++i)
				FreeEffectEntity(cl.Effects[index].u.Teleporter.entity_index[i]);
			break;

		case CE_HWSHEEPINATOR:
			for (i=0;i<5;++i)
				FreeEffectEntity(cl.Effects[index].u.Xbow.ent[i]);
			break;

		case CE_HWXBOWSHOOT:
			for (i=0;i<cl.Effects[index].u.Xbow.bolts;++i)
				FreeEffectEntity(cl.Effects[index].u.Xbow.ent[i]);
			break;

		case CE_TELEPORTERBODY:
			FreeEffectEntity(cl.Effects[index].u.Teleporter.entity_index[0]);
			break;
		...
		case CE_HWRAVENPOWER:
			FreeEffectEntity(cl.Effects[index].u.Missile.entity_index);
			break;
		case CE_TRIPMINESTILL:
//			Con_DPrintf("Ditching chain\n");
			FreeEffectEntity(cl.Effects[index].u.Chain.ent1);
			break;
		case CE_SCARABCHAIN:
		case CE_TRIPMINE:
			FreeEffectEntity(cl.Effects[index].u.Chain.ent1);
		...
		case CE_HWMISSILESTAR:
			FreeEffectEntity(cl.Effects[index].u.Star.ent2);
			//no break wanted here
		case CE_HWEIDOLONSTAR:
			FreeEffectEntity(cl.Effects[index].u.Star.ent1);
			FreeEffectEntity(cl.Effects[index].u.Star.entity_index);
...
void CL_ParseEffect(void)
{
	...
	{
		case CE_RAIN:
			cl.Effects[index].u.Rain.min_org[0] = MSG_ReadCoord();
			cl.Effects[index].u.Rain.min_org[1] = MSG_ReadCoord();
			cl.Effects[index].u.Rain.min_org[2] = MSG_ReadCoord();
			cl.Effects[index].u.Rain.max_org[0] = MSG_ReadCoord();
			cl.Effects[index].u.Rain.max_org[1] = MSG_ReadCoord();
			cl.Effects[index].u.Rain.max_org[2] = MSG_ReadCoord();
			cl.Effects[index].u.Rain.e_size[0] = MSG_ReadCoord();
			cl.Effects[index].u.Rain.e_size[1] = MSG_ReadCoord();
			cl.Effects[index].u.Rain.e_size[2] = MSG_ReadCoord();
			cl.Effects[index].u.Rain.dir[0] = MSG_ReadCoord();
			cl.Effects[index].u.Rain.dir[1] = MSG_ReadCoord();
			cl.Effects[index].u.Rain.dir[2] = MSG_ReadCoord();
			cl.Effects[index].u.Rain.color = MSG_ReadShort();
			cl.Effects[index].u.Rain.count = MSG_ReadShort();
			cl.Effects[index].u.Rain.wait = MSG_ReadFloat();
			break;

		case CE_FOUNTAIN:
			cl.Effects[index].u.Fountain.pos[0] = MSG_ReadCoord ();
			cl.Effects[index].u.Fountain.pos[1] = MSG_ReadCoord ();
			cl.Effects[index].u.Fountain.pos[2] = MSG_ReadCoord ();
			cl.Effects[index].u.Fountain.angle[0] = MSG_ReadAngle ();
			cl.Effects[index].u.Fountain.angle[1] = MSG_ReadAngle ();
			cl.Effects[index].u.Fountain.angle[2] = MSG_ReadAngle ();
			cl.Effects[index].u.Fountain.movedir[0] = MSG_ReadCoord ();
			cl.Effects[index].u.Fountain.movedir[1] = MSG_ReadCoord ();
			cl.Effects[index].u.Fountain.movedir[2] = MSG_ReadCoord ();
			cl.Effects[index].u.Fountain.color = MSG_ReadShort ();
			cl.Effects[index].u.Fountain.cnt = MSG_ReadByte ();
			AngleVectors (cl.Effects[index].u.Fountain.angle, 
						  cl.Effects[index].u.Fountain.vforward,
						  cl.Effects[index].u.Fountain.vright,
						  cl.Effects[index].u.Fountain.vup);
			break;

		case CE_QUAKE:
			cl.Effects[index].u.Quake.origin[0] = MSG_ReadCoord ();
			cl.Effects[index].u.Quake.origin[1] = MSG_ReadCoord ();
			cl.Effects[index].u.Quake.origin[2] = MSG_ReadCoord ();
			cl.Effects[index].u.Quake.radius = MSG_ReadFloat ();
			break;
		...
		case CE_ONFIRE:
		case CE_RIPPLE:
			cl.Effects[index].u.Smoke.origin[0] = MSG_ReadCoord ();
			cl.Effects[index].u.Smoke.origin[1] = MSG_ReadCoord ();
			cl.Effects[index].u.Smoke.origin[2] = MSG_ReadCoord ();

			cl.Effects[index].u.Smoke.velocity[0] = MSG_ReadFloat ();
			cl.Effects[index].u.Smoke.velocity[1] = MSG_ReadFloat ();
			cl.Effects[index].u.Smoke.velocity[2] = MSG_ReadFloat ();

			cl.Effects[index].u.Smoke.framelength = MSG_ReadFloat ();

			if ((cl.Effects[index].u.Smoke.entity_index = NewEffectEntity()) != -1)
			{
				ent = &EffectEntities[cl.Effects[index].u.Smoke.entity_index];
				VectorCopy(cl.Effects[index].u.Smoke.origin, ent->origin);
				...
				else if (cl.Effects[index].type == CE_ONFIRE)
				{
					...
					ent->abslight = 1;
					ent->frame = cl.Effects[index].u.Smoke.frame;
				}
				else if (cl.Effects[index].type == CE_RIPPLE)
				{
					if(cl.Effects[index].u.Smoke.framelength==2)
					{
						R_SplashParticleEffect (cl.Effects[index].u.Smoke.origin, 200, 406+rand()%8, pt_slowgrav, 40);//splash
						S_StartSound (TempSoundChannel(), 1, cl_fxsfx_splash, cl.Effects[index].u.Smoke.origin, 1, 1);
					}
					else if(cl.Effects[index].u.Smoke.framelength==1)
						R_SplashParticleEffect (cl.Effects[index].u.Smoke.origin, 100, 406+rand()%8, pt_slowgrav, 20);//splash
					else
						S_StartSound (TempSoundChannel(), 1, cl_fxsfx_ripple, cl.Effects[index].u.Smoke.origin, 1, 1);
					cl.Effects[index].u.Smoke.framelength=0.05;
				...
				{
					S_StartSound (TempSoundChannel(), 1, cl_fxsfx_ravenfire, cl.Effects[index].u.Smoke.origin, 1, 1);

					if ((cl.Effects[index].u.Smoke.entity_index2 = NewEffectEntity()) != -1)
					{
						ent = &EffectEntities[cl.Effects[index].u.Smoke.entity_index2];
						VectorCopy(cl.Effects[index].u.Smoke.origin, ent->origin);
						ent->model = Mod_ForName("models/telesmk1.spr", true);
						ent->drawflags = DRF_TRANSLUCENT;
					}
		...
		case CE_FIREWALL_LARGE:
			...
			if ((cl.Effects[index].u.Smoke.entity_index = NewEffectEntity()) != -1)
			{
				...
				{
					ent->model = Mod_ForName("models/sm_expld.spr", true);
					S_StartSound (TempSoundChannel(), 1, cl_fxsfx_explode, cl.Effects[index].u.Smoke.origin, 1, 1);
		...
		case CE_HWSPLITFLASH:
		case CE_RED_FLASH:
			cl.Effects[index].u.Flash.origin[0] = MSG_ReadCoord ();
			cl.Effects[index].u.Flash.origin[1] = MSG_ReadCoord ();
			cl.Effects[index].u.Flash.origin[2] = MSG_ReadCoord ();
			cl.Effects[index].u.Flash.reverse = 0;
			if ((cl.Effects[index].u.Flash.entity_index = NewEffectEntity()) != -1)
			{
				ent = &EffectEntities[cl.Effects[index].u.Flash.entity_index];
				VectorCopy(cl.Effects[index].u.Flash.origin, ent->origin);
				...
				{
					ent->model = Mod_ForName("models/sm_blue.spr", true);
					S_StartSound (TempSoundChannel(), 1, cl_fxsfx_ravensplit, cl.Effects[index].u.Flash.origin, 1, 1);
				}
				ent->drawflags = DRF_TRANSLUCENT;
		...
		case CE_RIDER_DEATH:
			cl.Effects[index].u.RD.origin[0] = MSG_ReadCoord ();
			cl.Effects[index].u.RD.origin[1] = MSG_ReadCoord ();
			cl.Effects[index].u.RD.origin[2] = MSG_ReadCoord ();
			break;

		case CE_TELEPORTERPUFFS:
			cl.Effects[index].u.Teleporter.origin[0] = MSG_ReadCoord ();
			cl.Effects[index].u.Teleporter.origin[1] = MSG_ReadCoord ();
			cl.Effects[index].u.Teleporter.origin[2] = MSG_ReadCoord ();
				
			cl.Effects[index].u.Teleporter.framelength = .05;
			dir = 0;
			for (i=0;i<8;++i)
			{		
				if ((cl.Effects[index].u.Teleporter.entity_index[i] = NewEffectEntity()) != -1)
				{
					ent = &EffectEntities[cl.Effects[index].u.Teleporter.entity_index[i]];
					VectorCopy(cl.Effects[index].u.Teleporter.origin, ent->origin);
					...
					cl.Effects[index].u.Teleporter.velocity[i][0] = 10*cosval;
					cl.Effects[index].u.Teleporter.velocity[i][1] = 10*sinval;
					cl.Effects[index].u.Teleporter.velocity[i][2] = 0;
					dir += 45;
		...
		case CE_TELEPORTERBODY:
			...
			cl.Effects[index].u.Teleporter.velocity[0][0] = MSG_ReadFloat ();
			cl.Effects[index].u.Teleporter.velocity[0][1] = MSG_ReadFloat ();
			cl.Effects[index].u.Teleporter.velocity[0][2] = MSG_ReadFloat ();

			skinnum = MSG_ReadFloat ();
			
			cl.Effects[index].u.Teleporter.framelength = .05;
			dir = 0;
			if ((cl.Effects[index].u.Teleporter.entity_index[0] = NewEffectEntity()) != -1)
			{
				ent = &EffectEntities[cl.Effects[index].u.Teleporter.entity_index[0]];
				VectorCopy(cl.Effects[index].u.Teleporter.origin, ent->origin);
		...
		case CE_BONESHRAPNEL:
		case CE_HWBONEBALL:
			cl.Effects[index].u.Missile.origin[0] = MSG_ReadCoord ();
			cl.Effects[index].u.Missile.origin[1] = MSG_ReadCoord ();
			cl.Effects[index].u.Missile.origin[2] = MSG_ReadCoord ();

			cl.Effects[index].u.Missile.velocity[0] = MSG_ReadFloat ();
			cl.Effects[index].u.Missile.velocity[1] = MSG_ReadFloat ();
			cl.Effects[index].u.Missile.velocity[2] = MSG_ReadFloat ();

			cl.Effects[index].u.Missile.angle[0] = MSG_ReadFloat ();
			cl.Effects[index].u.Missile.angle[1] = MSG_ReadFloat ();
			cl.Effects[index].u.Missile.angle[2] = MSG_ReadFloat ();

			cl.Effects[index].u.Missile.avelocity[0] = MSG_ReadFloat ();
			cl.Effects[index].u.Missile.avelocity[1] = MSG_ReadFloat ();
			cl.Effects[index].u.Missile.avelocity[2] = MSG_ReadFloat ();

			if ((cl.Effects[index].u.Missile.entity_index = NewEffectEntity()) != -1)
			{
				ent = &EffectEntities[cl.Effects[index].u.Missile.entity_index];
				VectorCopy(cl.Effects[index].u.Missile.origin, ent->origin);
				VectorCopy(cl.Effects[index].u.Missile.angle, ent->angles);
				...
				else if (cl.Effects[index].type == CE_HWBONEBALL)
				{
					...
					S_StartSound (TempSoundChannel(), 1, cl_fxsfx_bonefpow, cl.Effects[index].u.Missile.origin, 1, 1);
				}
			}
			else
				ImmediateFree = true;
		...
		case CE_HWRAVENPOWER:
			...
			cl.Effects[index].u.Missile.velocity[1] = MSG_ReadFloat ();
			cl.Effects[index].u.Missile.velocity[2] = MSG_ReadFloat ();
			vectoangles(cl.Effects[index].u.Missile.velocity, cl.Effects[index].u.Missile.angle);
			...
			if ((cl.Effects[index].u.Missile.entity_index = NewEffectEntity()) != -1)
			{
				...
				{
					cl.Effects[index].u.Missile.avelocity[2] = 1000;
					ent->model = Mod_ForName("models/vindsht1.mdl", true);
				}
				else if (cl.Effects[index].type == CE_BONESHARD)
				{
					cl.Effects[index].u.Missile.avelocity[0] = (rand() % 1554) - 777;
					ent->model = Mod_ForName("models/boneshot.mdl", true);
					S_StartSound (TempSoundChannel(), 1, cl_fxsfx_bone, cl.Effects[index].u.Missile.origin, 1, 1);
				}
				else if (cl.Effects[index].type == CE_HWRAVENPOWER)
				{
					ent->model = Mod_ForName("models/ravproj.mdl", true);
					S_StartSound (TempSoundChannel(), 1, cl_fxsfx_ravengo, cl.Effects[index].u.Missile.origin, 1, 1);
				}
			}
			else
				ImmediateFree = true;
			break;
		case CE_DEATHBUBBLES:
			cl.Effects[index].u.Bubble.owner = MSG_ReadShort();
			cl.Effects[index].u.Bubble.offset[0] = MSG_ReadByte();
			cl.Effects[index].u.Bubble.offset[1] = MSG_ReadByte();
			cl.Effects[index].u.Bubble.offset[2] = MSG_ReadByte();
			cl.Effects[index].u.Bubble.count = MSG_ReadByte();//num of bubbles
			cl.Effects[index].u.Bubble.time_amount = 0;
			break;
		case CE_HWXBOWSHOOT:
			...
			cl.Effects[index].u.Xbow.angle[0] = MSG_ReadAngle ();
			cl.Effects[index].u.Xbow.angle[1] = MSG_ReadAngle ();
			cl.Effects[index].u.Xbow.angle[2] = 0;//MSG_ReadFloat ();

			cl.Effects[index].u.Xbow.bolts = MSG_ReadByte();
			cl.Effects[index].u.Xbow.randseed = MSG_ReadByte();

			cl.Effects[index].u.Xbow.turnedbolts = MSG_ReadByte();

			cl.Effects[index].u.Xbow.activebolts= MSG_ReadByte();

			setseed(cl.Effects[index].u.Xbow.randseed);

			AngleVectors (cl.Effects[index].u.Xbow.angle, forward, right, up);

			VectorNormalize(forward);
			VectorCopy(forward, cl.Effects[index].u.Xbow.velocity);
//			VectorScale(forward, 1000, cl.Effects[index].u.Xbow.velocity);

			if (cl.Effects[index].u.Xbow.bolts == 3)
			{
				S_StartSound (TempSoundChannel(), 1, cl_fxsfx_xbowshoot, origin, 1, 1);
			}
			else if (cl.Effects[index].u.Xbow.bolts == 5)
			{
			...
			for (i=0;i<cl.Effects[index].u.Xbow.bolts;i++)
			{
				cl.Effects[index].u.Xbow.gonetime[i] = 1 + seedrand()*2;
				cl.Effects[index].u.Xbow.state[i] = 0;

				if ((1<<i)&	cl.Effects[index].u.Xbow.turnedbolts)
				{
					cl.Effects[index].u.Xbow.origin[i][0]=MSG_ReadCoord();
					cl.Effects[index].u.Xbow.origin[i][1]=MSG_ReadCoord();
					cl.Effects[index].u.Xbow.origin[i][2]=MSG_ReadCoord();
					vtemp[0]=MSG_ReadAngle();
					vtemp[1]=MSG_ReadAngle();
					vtemp[2]=0;
					AngleVectors (vtemp, forward2, right2, up2);
					VectorScale(forward2, 800 + seedrand()*500, cl.Effects[index].u.Xbow.vel[i]);
				}
				else
				{
					VectorScale(forward, 800 + seedrand()*500, cl.Effects[index].u.Xbow.vel[i]);

					VectorScale(right,i*100-(cl.Effects[index].u.Xbow.bolts-1)*50,vtemp);

					//this should only be done for deathmatch:
					VectorScale(vtemp,0.333,vtemp);

					VectorAdd(cl.Effects[index].u.Xbow.vel[i],vtemp,cl.Effects[index].u.Xbow.vel[i]);

					//start me off a bit out
					VectorScale(vtemp,0.05,cl.Effects[index].u.Xbow.origin[i]);
					VectorAdd(origin,cl.Effects[index].u.Xbow.origin[i],cl.Effects[index].u.Xbow.origin[i]);
				}

				if ((cl.Effects[index].u.Xbow.ent[i] = NewEffectEntity()) != -1)
				{
					ent = &EffectEntities[cl.Effects[index].u.Xbow.ent[i]];
					VectorCopy(cl.Effects[index].u.Xbow.origin[i], ent->origin);
					vectoangles(cl.Effects[index].u.Xbow.vel[i],ent->angles);
					if (cl.Effects[index].u.Xbow.bolts == 5)
		...
		case CE_HWSHEEPINATOR:
			...
			cl.Effects[index].u.Xbow.bolts = 5;
			cl.Effects[index].u.Xbow.randseed = 0;
...
//			S_StartSound (TempSoundChannel(), 1, cl_fxsfx_xbowshoot, origin, 1, 1);
			...
			for (i=0;i<cl.Effects[index].u.Xbow.bolts;i++)
			{
				cl.Effects[index].u.Xbow.gonetime[i] = 0;
				cl.Effects[index].u.Xbow.state[i] = 0;
				...
				if ((1<<i)&	cl.Effects[index].u.Xbow.turnedbolts)
				{
					...
					vtemp[2]=0;
					AngleVectors (vtemp, forward2, right2, up2);
					VectorScale(forward2, 700, cl.Effects[index].u.Xbow.vel[i]);
				}
				else
				{
					VectorCopy(origin,cl.Effects[index].u.Xbow.origin[i]);
					VectorScale(forward, 700, cl.Effects[index].u.Xbow.vel[i]);
		...
		case CE_HWDRILLA:
			...
			cl.Effects[index].u.Missile.angle[0] = MSG_ReadAngle ();
			cl.Effects[index].u.Missile.angle[1] = MSG_ReadAngle ();
			cl.Effects[index].u.Missile.angle[2] = 0;

			cl.Effects[index].u.Missile.speed = MSG_ReadShort();

			S_StartSound (TempSoundChannel(), 1, cl_fxsfx_drillashoot, cl.Effects[index].u.Missile.origin, 1, 1);

			AngleVectors(cl.Effects[index].u.Missile.angle,cl.Effects[index].u.Missile.velocity,right,up);

			VectorScale(cl.Effects[index].u.Missile.velocity,cl.Effects[index].u.Missile.speed,cl.Effects[index].u.Missile.velocity);
			...
			break;
		case CE_SCARABCHAIN:
			cl.Effects[index].u.Chain.origin[0] = MSG_ReadCoord ();
			cl.Effects[index].u.Chain.origin[1] = MSG_ReadCoord ();
			cl.Effects[index].u.Chain.origin[2] = MSG_ReadCoord ();

			cl.Effects[index].u.Chain.owner = MSG_ReadShort ();
			cl.Effects[index].u.Chain.tag = MSG_ReadByte();
			cl.Effects[index].u.Chain.material = cl.Effects[index].u.Chain.owner>>12;
			cl.Effects[index].u.Chain.owner &= 0x0fff;
			cl.Effects[index].u.Chain.height = 16;//MSG_ReadByte ();

			cl.Effects[index].u.Chain.sound_time = cl.time;

			cl.Effects[index].u.Chain.state = 0;//state 0: move slowly toward owner

			S_StartSound (TempSoundChannel(), 1, cl_fxsfx_scarabwhoosh, cl.Effects[index].u.Chain.origin, 1, 1);

			if ((cl.Effects[index].u.Chain.ent1 = NewEffectEntity()) != -1)
			{
				ent = &EffectEntities[cl.Effects[index].u.Chain.ent1];
				VectorCopy(cl.Effects[index].u.Chain.origin, ent->origin);
		...
		case CE_TRIPMINE:
			...
			cl.Effects[index].u.Chain.velocity[0] = MSG_ReadFloat ();
			cl.Effects[index].u.Chain.velocity[1] = MSG_ReadFloat ();
			cl.Effects[index].u.Chain.velocity[2] = MSG_ReadFloat ();
...
//			Con_DPrintf("Allocating chain effect...\n");
			...
			{
				ent = &EffectEntities[cl.Effects[index].u.Chain.ent1];
				VectorCopy(cl.Effects[index].u.Chain.velocity, ent->origin);
				ent->model = Mod_ForName("models/twspike.mdl", true);
			}
			else
			{
//				Con_DPrintf("ERROR: Couldn't allocate chain effect!\n");
		...
		case CE_HWMISSILESTAR:
		case CE_HWEIDOLONSTAR:
			cl.Effects[index].u.Star.origin[0] = MSG_ReadCoord ();
			cl.Effects[index].u.Star.origin[1] = MSG_ReadCoord ();
			cl.Effects[index].u.Star.origin[2] = MSG_ReadCoord ();

			cl.Effects[index].u.Star.velocity[0] = MSG_ReadFloat ();
			cl.Effects[index].u.Star.velocity[1] = MSG_ReadFloat ();
			cl.Effects[index].u.Star.velocity[2] = MSG_ReadFloat ();
			vectoangles(cl.Effects[index].u.Star.velocity, cl.Effects[index].u.Star.angle);
			cl.Effects[index].u.Missile.avelocity[2] = 300 + rand()%300;

			if ((cl.Effects[index].u.Star.entity_index = NewEffectEntity()) != -1)
			{
				ent = &EffectEntities[cl.Effects[index].u.Star.entity_index];
				VectorCopy(cl.Effects[index].u.Star.origin, ent->origin);
				VectorCopy(cl.Effects[index].u.Star.angle, ent->angles);
				ent->model = Mod_ForName("models/ball.mdl", true);
			}
			else
				ImmediateFree = true;

			cl.Effects[index].u.Star.scaleDir = 1;
			cl.Effects[index].u.Star.scale = 0.3;
			
			if ((cl.Effects[index].u.Star.ent1 = NewEffectEntity()) != -1)
			{
				ent = &EffectEntities[cl.Effects[index].u.Star.ent1];
				VectorCopy(cl.Effects[index].u.Star.origin, ent->origin);
				ent->drawflags |= MLS_ABSLIGHT;
				ent->abslight = 0.5;
				ent->angles[2] = 90;
			...
			if(cl.Effects[index].type == CE_HWMISSILESTAR)
			{
				if ((cl.Effects[index].u.Star.ent2 = NewEffectEntity()) != -1)
				{
					ent = &EffectEntities[cl.Effects[index].u.Star.ent2];
					VectorCopy(cl.Effects[index].u.Star.origin, ent->origin);
					ent->model = Mod_ForName("models/star.mdl", true);
					ent->drawflags |= MLS_ABSLIGHT;
					ent->abslight = 0.5;
...
void CL_EndEffect(void)
{
	...
	{
	case CE_HWRAVENPOWER:
		if(cl.Effects[index].u.Missile.entity_index > -1)
		{
			ent = &EffectEntities[cl.Effects[index].u.Missile.entity_index];
							...
							// or else client is sure to crash.	
{
	...
	if (cl.Effects[index].type==type)
		...
		case CE_SCARABCHAIN://attach to new guy or retract if new guy is world
			...
			if (cl.Effects[index].type==type)
			{
				cl.Effects[index].u.Chain.material = curEnt>>12;
				curEnt &= 0x0fff;

				if (curEnt)
				{
					cl.Effects[index].u.Chain.state = 1;
					cl.Effects[index].u.Chain.owner = curEnt;
					es = FindState(cl.Effects[index].u.Chain.owner);
					if (es)
					{
						ent = &EffectEntities[cl.Effects[index].u.Chain.ent1];
						XbowImpactPuff(ent->origin, cl.Effects[index].u.Chain.material);
					}
				}
				else
					cl.Effects[index].u.Chain.state = 2;
			}
			break;
		case CE_HWXBOWSHOOT:
			...
			{
				cl.Effects[index].u.Xbow.activebolts &= ~(1<<curEnt);
				dist = MSG_ReadCoord();
				if (cl.Effects[index].u.Xbow.ent[curEnt]!= -1)
				{
					ent = &EffectEntities[cl.Effects[index].u.Xbow.ent[curEnt]];

					//make sure bolt is in correct position
					VectorCopy(cl.Effects[index].u.Xbow.vel[curEnt],forward);
					VectorNormalize(forward);
					VectorScale(forward,dist,forward);
					VectorAdd(cl.Effects[index].u.Xbow.origin[curEnt],forward,ent->origin);
					...
					if (takedamage)
					{
						cl.Effects[index].u.Xbow.gonetime[curEnt] = cl.time;
					}
					else
					{
						cl.Effects[index].u.Xbow.gonetime[curEnt] += cl.time;
					...
					CLTENT_XbowImpact(ent->origin, forward, material, takedamage, cl.Effects[index].u.Xbow.bolts);
				}
			}
			else
			{
				if (cl.Effects[index].u.Xbow.ent[curEnt]!=-1)
				{
					...
					{
						cl.Effects[index].u.Xbow.origin[curEnt][0]=MSG_ReadCoord();
						cl.Effects[index].u.Xbow.origin[curEnt][1]=MSG_ReadCoord();
						cl.Effects[index].u.Xbow.origin[curEnt][2]=MSG_ReadCoord();
					}

					AngleVectors(ent->angles,forward,right,up);
					speed = Length(cl.Effects[index].u.Xbow.vel[curEnt]);
					VectorScale(forward,speed,cl.Effects[index].u.Xbow.vel[curEnt]);
					VectorCopy(cl.Effects[index].u.Xbow.origin[curEnt],ent->origin);
				}
				else
				{
					pos[0] = MSG_ReadAngle();
		...
		case CE_HWDRILLA:
			...
			if (revisionCode == 0)//impact
			{
				...
				//lil bits at exit
				VectorCopy(cl.Effects[index].u.Missile.velocity,forward);
				VectorNormalize(forward);
			...
			else//turn
			{
				if (cl.Effects[index].u.Missile.entity_index!=-1)
				{
					...
					AngleVectors(ent->angles,forward,right,up);
					speed = Length(cl.Effects[index].u.Missile.velocity);
					VectorScale(forward,speed,cl.Effects[index].u.Missile.velocity);
					VectorCopy(cl.Effects[index].u.Missile.origin,ent->origin);
				}
				else
				{
...
void CL_TurnEffect(void)
{
	...
	case CE_HWBONEBALL:
		if(cl.Effects[index].u.Missile.entity_index > -1)
		{
			...
			VectorCopy(vel, cl.Effects[index].u.Missile.velocity);
			vectoangles(cl.Effects[index].u.Missile.velocity, cl.Effects[index].u.Missile.angle);
		}
		break;
	case CE_HWMISSILESTAR:
	case CE_HWEIDOLONSTAR:
		if(cl.Effects[index].u.Star.entity_index > -1)
		{
			ent = &EffectEntities[cl.Effects[index].u.Star.entity_index];
			UpdateMissilePath(ent->origin, pos, vel, time);
			VectorCopy(vel, cl.Effects[index].u.Star.velocity);
		}
		break;
	case 0:
		// create a clc message to retrieve effect information
...
//	Con_Printf("Here at %f\n",cl.time);
	...
	for(index=0;index<MAX_EFFECTS;index++)
	{
		...
		{
			case CE_RAIN:
				org[0] = cl.Effects[index].u.Rain.min_org[0];
				org[1] = cl.Effects[index].u.Rain.min_org[1];
				org[2] = cl.Effects[index].u.Rain.max_org[2];

				org2[0] = cl.Effects[index].u.Rain.e_size[0];
				org2[1] = cl.Effects[index].u.Rain.e_size[1];
				org2[2] = cl.Effects[index].u.Rain.e_size[2];

				x_dir = cl.Effects[index].u.Rain.dir[0];
				y_dir = cl.Effects[index].u.Rain.dir[1];
				
				cl.Effects[index].u.Rain.next_time += frametime;
				if (cl.Effects[index].u.Rain.next_time >= cl.Effects[index].u.Rain.wait)
				{		
					R_RainEffect(org,org2,x_dir,y_dir,cl.Effects[index].u.Rain.color,
						cl.Effects[index].u.Rain.count);
					cl.Effects[index].u.Rain.next_time = 0;
				}
				break;

			case CE_FOUNTAIN:
				mymin[0] = (-3 * cl.Effects[index].u.Fountain.vright[0] * cl.Effects[index].u.Fountain.movedir[0]) +
						   (-3 * cl.Effects[index].u.Fountain.vforward[0] * cl.Effects[index].u.Fountain.movedir[1]) +
						   (2 * cl.Effects[index].u.Fountain.vup[0] * cl.Effects[index].u.Fountain.movedir[2]);
				mymin[1] = (-3 * cl.Effects[index].u.Fountain.vright[1] * cl.Effects[index].u.Fountain.movedir[0]) +
						   (-3 * cl.Effects[index].u.Fountain.vforward[1] * cl.Effects[index].u.Fountain.movedir[1]) +
						   (2 * cl.Effects[index].u.Fountain.vup[1] * cl.Effects[index].u.Fountain.movedir[2]);
				mymin[2] = (-3 * cl.Effects[index].u.Fountain.vright[2] * cl.Effects[index].u.Fountain.movedir[0]) +
						   (-3 * cl.Effects[index].u.Fountain.vforward[2] * cl.Effects[index].u.Fountain.movedir[1]) +
						   (2 * cl.Effects[index].u.Fountain.vup[2] * cl.Effects[index].u.Fountain.movedir[2]);
				mymin[0] *= 15;
				mymin[1] *= 15;
				mymin[2] *= 15;

				mymax[0] = (3 * cl.Effects[index].u.Fountain.vright[0] * cl.Effects[index].u.Fountain.movedir[0]) +
						   (3 * cl.Effects[index].u.Fountain.vforward[0] * cl.Effects[index].u.Fountain.movedir[1]) +
						   (10 * cl.Effects[index].u.Fountain.vup[0] * cl.Effects[index].u.Fountain.movedir[2]);
				mymax[1] = (3 * cl.Effects[index].u.Fountain.vright[1] * cl.Effects[index].u.Fountain.movedir[0]) +
						   (3 * cl.Effects[index].u.Fountain.vforward[1] * cl.Effects[index].u.Fountain.movedir[1]) +
						   (10 * cl.Effects[index].u.Fountain.vup[1] * cl.Effects[index].u.Fountain.movedir[2]);
				mymax[2] = (3 * cl.Effects[index].u.Fountain.vright[2] * cl.Effects[index].u.Fountain.movedir[0]) +
						   (3 * cl.Effects[index].u.Fountain.vforward[2] * cl.Effects[index].u.Fountain.movedir[1]) +
						   (10 * cl.Effects[index].u.Fountain.vup[2] * cl.Effects[index].u.Fountain.movedir[2]);
				mymax[0] *= 15;
				mymax[1] *= 15;
				mymax[2] *= 15;

				R_RunParticleEffect2 (cl.Effects[index].u.Fountain.pos,mymin,mymax,
					                  cl.Effects[index].u.Fountain.color,2,cl.Effects[index].u.Fountain.cnt);

/*				memset(&test,0,sizeof(test));
				trace = SV_Move (cl.Effects[index].u.Fountain.pos, mymin, mymax, mymin, false, &test);
				Con_Printf("Fraction is %f\n",trace.fraction);*/
				break;

			case CE_QUAKE:
				R_RunQuakeEffect (cl.Effects[index].u.Quake.origin,cl.Effects[index].u.Quake.radius);
				break;

			case CE_RIPPLE:
				cl.Effects[index].u.Smoke.time_amount += frametime;
				ent = &EffectEntities[cl.Effects[index].u.Smoke.entity_index];

				smoketime = cl.Effects[index].u.Smoke.framelength;
				if (!smoketime)
					smoketime = HX_FRAME_TIME*2;

				while(cl.Effects[index].u.Smoke.time_amount >= smoketime&&ent->scale<250)
				{
					ent->frame++;
					ent->angles[1]+=1;
					cl.Effects[index].u.Smoke.time_amount -= smoketime;
				}
			...
			case CE_ONFIRE:
				...
				ent->origin[0] += (frametime/smoketime) * cl.Effects[index].u.Smoke.velocity[0];
				ent->origin[1] += (frametime/smoketime) * cl.Effects[index].u.Smoke.velocity[1];
				ent->origin[2] += (frametime/smoketime) * cl.Effects[index].u.Smoke.velocity[2];
	
				i=0;
				while(cl.Effects[index].u.Smoke.time_amount >= smoketime)
				{
					ent->frame++;
					i++;
					cl.Effects[index].u.Smoke.time_amount -= smoketime;
			...
			case CE_FIREWALL_LARGE:
				...
				{
					while(cl.Effects[index].u.Smoke.time_amount >= HX_FRAME_TIME)
					{
						ent->frame++;
						cl.Effects[index].u.Smoke.time_amount -= HX_FRAME_TIME;
					}
				}
				else
				{
					while(cl.Effects[index].u.Smoke.time_amount >= HX_FRAME_TIME * 2)
					{
						ent->frame++;
						cl.Effects[index].u.Smoke.time_amount -= HX_FRAME_TIME * 2;
					}
				}
			...
			case CE_HWSPLITFLASH:
			case CE_RED_FLASH:
				cl.Effects[index].u.Flash.time_amount += frametime;
				ent = &EffectEntities[cl.Effects[index].u.Flash.entity_index];

				while(cl.Effects[index].u.Flash.time_amount >= HX_FRAME_TIME)
				{
					if (!cl.Effects[index].u.Flash.reverse)
					{
						if (ent->frame >= ent->model->numframes-1)  // Ran through forward animation
						{
							cl.Effects[index].u.Flash.reverse = 1;
							ent->frame--;
						}
						else
							ent->frame++;
					...
					cl.Effects[index].u.Flash.time_amount -= HX_FRAME_TIME;
				}

				if ((ent->frame <= 0) && (cl.Effects[index].u.Flash.reverse))
				{
					CL_FreeEffect(index);
				}
				else
			...
			case CE_RIDER_DEATH:
				cl.Effects[index].u.RD.time_amount += frametime;
				if (cl.Effects[index].u.RD.time_amount >= 1)
				{
					cl.Effects[index].u.RD.stage++;
					cl.Effects[index].u.RD.time_amount -= 1;
				}

				VectorCopy(cl.Effects[index].u.RD.origin,org);
				org[0] += sin(cl.Effects[index].u.RD.time_amount * 2 * M_PI) * 30;
				org[1] += cos(cl.Effects[index].u.RD.time_amount * 2 * M_PI) * 30;

				if (cl.Effects[index].u.RD.stage <= 6)
				{
					RiderParticle(cl.Effects[index].u.RD.stage+1,org);
				}
				else
				{
					// To set the rider's origin point for the particles
					RiderParticle(0,org);
					if (cl.Effects[index].u.RD.stage == 7) 
					{
					...
					else if (cl.Effects[index].u.RD.stage > 13) 
					{
//						cl.Effects[index].u.RD.stage = 0;
						CL_FreeEffect(index);
					}
				}
				break;
			...
			case CE_TELEPORTERPUFFS:
				cl.Effects[index].u.Teleporter.time_amount += frametime;
				smoketime = cl.Effects[index].u.Teleporter.framelength;

				ent = &EffectEntities[cl.Effects[index].u.Teleporter.entity_index[0]];
				while(cl.Effects[index].u.Teleporter.time_amount >= HX_FRAME_TIME)
				{
					ent->frame++;
					cl.Effects[index].u.Teleporter.time_amount -= HX_FRAME_TIME;
				}
				cur_frame = ent->frame;
				...
				for (i=0;i<8;++i)
				{
					...
					ent->origin[0] += (frametime/smoketime) * cl.Effects[index].u.Teleporter.velocity[i][0];
					ent->origin[1] += (frametime/smoketime) * cl.Effects[index].u.Teleporter.velocity[i][1];
					ent->origin[2] += (frametime/smoketime) * cl.Effects[index].u.Teleporter.velocity[i][2];
					ent->frame = cur_frame;
			...
			case CE_HWDRILLA:
				cl.Effects[index].u.Missile.time_amount += frametime;
				ent = &EffectEntities[cl.Effects[index].u.Missile.entity_index];
				...
				ent->angles[0] += frametime * cl.Effects[index].u.Missile.avelocity[0];
				ent->angles[1] += frametime * cl.Effects[index].u.Missile.avelocity[1];
				ent->angles[2] += frametime * cl.Effects[index].u.Missile.avelocity[2];

				VectorCopy(ent->origin,old_origin);

				ent->origin[0] += frametime * cl.Effects[index].u.Missile.velocity[0];
				ent->origin[1] += frametime * cl.Effects[index].u.Missile.velocity[1];
				ent->origin[2] += frametime * cl.Effects[index].u.Missile.velocity[2];
				...
				break;
			case CE_HWXBOWSHOOT:
				cl.Effects[index].u.Xbow.time_amount += frametime;
				for (i=0;i<cl.Effects[index].u.Xbow.bolts;i++)
				{
					if (cl.Effects[index].u.Xbow.ent[i] != -1)//only update valid effect ents
					{
						if (cl.Effects[index].u.Xbow.activebolts & (1<<i))//bolt in air, simply update position
						{
							ent = &EffectEntities[cl.Effects[index].u.Xbow.ent[i]];

							ent->origin[0] += frametime * cl.Effects[index].u.Xbow.vel[i][0];
							ent->origin[1] += frametime * cl.Effects[index].u.Xbow.vel[i][1];
							ent->origin[2] += frametime * cl.Effects[index].u.Xbow.vel[i][2];

							CL_LinkEntity(ent);
						}
						else if (cl.Effects[index].u.Xbow.bolts == 5)//fiery bolts don't just go away
						{
							if (cl.Effects[index].u.Xbow.state[i] == 0)//waiting to explode state
							{
								if (cl.Effects[index].u.Xbow.gonetime[i] > cl.time)//fiery bolts stick around for a while
								{
									ent = &EffectEntities[cl.Effects[index].u.Xbow.ent[i]];
									CL_LinkEntity(ent);
								}
								else//when time's up on fiery guys, they explode
								{
									...
									cl.Effects[index].u.Xbow.state[i] = 1;

									ent = &EffectEntities[cl.Effects[index].u.Xbow.ent[i]];

									//move bolt back a little to make explosion look better
									VectorNormalize(cl.Effects[index].u.Xbow.vel[i]);
									VectorScale(cl.Effects[index].u.Xbow.vel[i],-8,cl.Effects[index].u.Xbow.vel[i]);
									VectorAdd(ent->origin,cl.Effects[index].u.Xbow.vel[i],ent->origin);
									...
									cl.Effects[index].u.Xbow.gonetime[i] = cl.time + HX_FRAME_TIME * 2;
							...
							else if (cl.Effects[index].u.Xbow.state[i] == 1)//fiery bolt exploding state
							{
								ent = &EffectEntities[cl.Effects[index].u.Xbow.ent[i]];

								//increment frame if it's time
								while(cl.Effects[index].u.Xbow.gonetime[i] <= cl.time)
								{
									ent->frame++;
									cl.Effects[index].u.Xbow.gonetime[i] += HX_FRAME_TIME * 0.75;
								}


								if (ent->frame >= ent->model->numframes)
								{
									cl.Effects[index].u.Xbow.state[i] = 2;//if anim is over, set me to inactive state
								}
								else
									CL_LinkEntity(ent);
							}
			...
			case CE_HWSHEEPINATOR:
				...
				break;
			case CE_DEATHBUBBLES:
				cl.Effects[index].u.Bubble.time_amount += frametime;
				if (cl.Effects[index].u.Bubble.time_amount > 0.1)//10 bubbles a sec
				{
					cl.Effects[index].u.Bubble.time_amount = 0;
					cl.Effects[index].u.Bubble.count--;
					es = FindState(cl.Effects[index].u.Bubble.owner);
					if (es)
					{
						VectorCopy(es->origin,org);
						VectorAdd(org,cl.Effects[index].u.Bubble.offset,org);
				...
				if (cl.Effects[index].u.Bubble.count <= 0)
					CL_FreeEffect(index);
				break;
			case CE_SCARABCHAIN:
				cl.Effects[index].u.Chain.time_amount += frametime;
				ent = &EffectEntities[cl.Effects[index].u.Chain.ent1];

				switch (cl.Effects[index].u.Chain.state)
				{
				case 0://zooming in toward owner
					es = FindState(cl.Effects[index].u.Chain.owner);
					if (cl.Effects[index].u.Chain.sound_time <= cl.time)
					{
						cl.Effects[index].u.Chain.sound_time = cl.time + 0.5;
						S_StartSound (TempSoundChannel(), 1, cl_fxsfx_scarabhome, ent->origin, 1, 1);
					}
					if (es)
					{
						VectorCopy(es->origin,org);
						org[2]+=cl.Effects[index].u.Chain.height;
						...
						if (fabs(VectorNormalize(org))<500*frametime)
						{
							...
							ent->origin[2] += cl.Effects[index].u.Chain.height;
							XbowImpactPuff(ent->origin, cl.Effects[index].u.Chain.material);
						}
						else
						{
				...
				case 2://unattaching, server needs to set this state
					...
					VectorSubtract(cl.Effects[index].u.Chain.origin,org,org);
					...
					else//done--flash & git outa here (change type to redflash)
					{
						...
						cl.Effects[index].u.Flash.entity_index = cl.Effects[index].u.Chain.ent1;
						cl.Effects[index].type = CE_RED_FLASH;
						VectorCopy(ent->origin,cl.Effects[index].u.Flash.origin);
						cl.Effects[index].u.Flash.reverse = 0;
						ent->model = Mod_ForName("models/redspt.spr", true);
						ent->frame = 0;
						ent->drawflags = DRF_TRANSLUCENT;
				...
				VectorCopy(cl.Effects[index].u.Chain.origin, org);
				VectorCopy(ent->origin, org2);
				CreateStream(TE_STREAM_CHAIN, cl.Effects[index].u.Chain.ent1, 1, cl.Effects[index].u.Chain.tag, 0.1, 0, org, org2);
...
//				if (cl.Effects[index].u.Chain.ent1 < 0)//fixme: remove this!!!
//					Con_DPrintf("OHSHITOHSHIT--bad chain ent\n");

				CL_LinkEntity(ent);
//				Con_DPrintf("Chain Ent at: %d %d %d\n",(int)cl.Effects[index].u.Chain.origin[0],(int)cl.Effects[index].u.Chain.origin[1],(int)cl.Effects[index].u.Chain.origin[2]);

				//damndamndamn--add stream stuff here!
				VectorCopy(cl.Effects[index].u.Chain.origin, org);
				VectorCopy(ent->origin, org2);
				CreateStream(TE_STREAM_CHAIN, cl.Effects[index].u.Chain.ent1, 1, 1, 0.1, 0, org, org2);
			...
			case CE_TRIPMINE:
				...
				ent->origin[0] += frametime * cl.Effects[index].u.Chain.velocity[0];
				ent->origin[1] += frametime * cl.Effects[index].u.Chain.velocity[1];
				ent->origin[2] += frametime * cl.Effects[index].u.Chain.velocity[2];
...
//		ent->angles[0] = cl.Effects[index].u.Missile.angle[0];
//		ent->angles[1] = cl.Effects[index].u.Missile.angle[1];
//		ent->angles[2] = cl.Effects[index].u.Missile.angle[2];
				...
				{
					while(cl.Effects[index].u.Missile.time_amount >= HX_FRAME_TIME)
					{
						ent->frame++;
						cl.Effects[index].u.Missile.time_amount -= HX_FRAME_TIME;
					}
			...
			case CE_HWEIDOLONSTAR:
				// update scale
				if(cl.Effects[index].u.Star.scaleDir)
				{
					cl.Effects[index].u.Star.scale += 0.05;
					if(cl.Effects[index].u.Star.scale >= 1)
					{
						cl.Effects[index].u.Star.scaleDir = 0;
					}
				}
				else
				{
					cl.Effects[index].u.Star.scale -= 0.05;
					if(cl.Effects[index].u.Star.scale <= 0.01)
					{
						cl.Effects[index].u.Star.scaleDir = 1;
					}
				}
				
				cl.Effects[index].u.Star.time_amount += frametime;
				ent = &EffectEntities[cl.Effects[index].u.Star.entity_index];

				ent->angles[0] += frametime * cl.Effects[index].u.Star.avelocity[0];
				ent->angles[1] += frametime * cl.Effects[index].u.Star.avelocity[1];
				ent->angles[2] += frametime * cl.Effects[index].u.Star.avelocity[2];

				ent->origin[0] += frametime * cl.Effects[index].u.Star.velocity[0];
				ent->origin[1] += frametime * cl.Effects[index].u.Star.velocity[1];
				ent->origin[2] += frametime * cl.Effects[index].u.Star.velocity[2];

				CL_LinkEntity(ent);
				
				if (cl.Effects[index].u.Star.ent1 != -1)
				{
					ent2= &EffectEntities[cl.Effects[index].u.Star.ent1];
					VectorCopy(ent->origin, ent2->origin);
					ent2->scale = cl.Effects[index].u.Star.scale;
				...
				if(cl.Effects[index].type == CE_HWMISSILESTAR)
				{
					if (cl.Effects[index].u.Star.ent2 != -1)
					{
						ent2 = &EffectEntities[cl.Effects[index].u.Star.ent2];
						VectorCopy(ent->origin, ent2->origin);
						ent2->scale = cl.Effects[index].u.Star.scale;
						ent2->angles[1] += frametime * -300;
						ent2->angles[2] += frametime * -400;
...
void CL_ParseMultiEffect(void)
{
	...
	case CE_HWRAVENPOWER:
		...
		for(count=0;count<3;count++)
		{
			...
			VectorCopy(orig, cl.Effects[index].u.Missile.origin);
			VectorCopy(vel, cl.Effects[index].u.Missile.velocity);
			vectoangles(cl.Effects[index].u.Missile.velocity, cl.Effects[index].u.Missile.angle);
			if ((cl.Effects[index].u.Missile.entity_index = NewEffectEntity()) != -1)
			{
 ...
 *
 * Revision 1.1.1.1  2001/11/09 17:04:38  theoddone33
 * Inital import
 *
 * 
