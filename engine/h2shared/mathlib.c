...
# pragma optimize( "", off )
#endif
void RotatePointAroundVector (vec3_t dst, const vec3_t axis, const vec3_t point,
			      float degrees)
{
	float		m[3][3];
	float		im[3][3];
	...
	dst[0] = DotProduct (rot[0], point);
	dst[1] = DotProduct (rot[1], point);
	dst[2] = DotProduct (rot[2], point);
}
